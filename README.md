...
We provide several example programs under `tests/test-progs/cim/src`. You can build them (alongside our cim standard library) using:
```sh
cd test-progs/cim/src

make                            # build everything
make test TEST=workloads        # build only CIM workloads
make test TEST=primitives       # build only CIM primitives
