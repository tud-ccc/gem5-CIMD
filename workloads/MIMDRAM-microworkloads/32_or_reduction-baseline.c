...
static uint8_t or_reduction8(uint8_t x) {
	x |= x >> 4;
	x |= x >> 2;
	x |= x >> 1;
	return x & 1;
...
static uint8_t or_reduction16(uint16_t x) {
	x |= x >> 8;
	x |= x >> 4;
	x |= x >> 2;
	x |= x >> 1;
...
static uint8_t or_reduction32(uint32_t x) {
	x |= x >> 16;
	x |= x >> 8;
	x |= x >> 4;
	x |= x >> 2;
...
static uint8_t or_reduction64(uint64_t x) {
	x |= x >> 32;
	x |= x >> 16;
	x |= x >> 8;
	x |= x >> 4;
		...
		switch(col_length) {
			case 8:
				for (int i = 0; i < num_vals; ++i) {
					output[i] = or_reduction8(v8[i]);
				}
				break;
			case 16:
				for (int i = 0; i < num_vals; ++i) {
					output[i] = or_reduction16(v16[i]);
				}
				break;
			case 32:
				for (int i = 0; i < num_vals; ++i) {
					output[i] = or_reduction32(v32[i]);
				}
				break;
			case 64:
				for (int i = 0; i < num_vals; ++i) {
					output[i] = or_reduction64(v64[i]);
				}
				break;
		}
   	}
