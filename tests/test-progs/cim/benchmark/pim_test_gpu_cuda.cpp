// GPU (CUDA) implementation of row operations benchmark
...
{
    if (argc >= 2) {
        size_t parsed_size = atoi(argv[1]);
        if (parsed_size > 0) {
            N_ELEMS = parsed_size;
    ...
    cout << "GPU (CUDA): Running " << op_names[op_id-1] << " (op_id=" << op_id << ")" << endl;
    ...
    CUDA_CHECK(cudaMalloc(&d_array1, N_ELEMS * sizeof(dtype)));
    CUDA_CHECK(cudaMalloc(&d_array2, N_ELEMS * sizeof(dtype)));

    CUDA_CHECK(cudaMemcpy(d_array1, array1_host, N_ELEMS * sizeof(dtype), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_array2, array2_host, N_ELEMS * sizeof(dtype), cudaMemcpyHostToDevice));
    ...
    switch(op_id) {
        case 1: {
            ...
            CUDA_CHECK(cudaMemcpy(array1_host, d_array1, N_ELEMS * sizeof(dtype), cudaMemcpyDeviceToHost));
    ...
    CUDA_CHECK(cudaFree(d_array1));
    CUDA_CHECK(cudaFree(d_array2));
    free(array1_host);
    free(array2_host);
